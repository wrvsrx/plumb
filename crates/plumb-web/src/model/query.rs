...
impl WebWorkspace {
    ...
    /// Web projection of the shared `next` shortlist.
    ///
    /// Both sections come from one `query_next` call on one workspace snapshot
    /// and one query instant, so the Web client never re-implements the split,
    /// the ordering, or the filters. The candidate limit defaults at this
    /// adapter boundary and the shared query clamps it to 1..=10; the in-flight
    /// section pages independently through its own cursor.
    pub fn query_next(
        &self,
        limit: Option<usize>,
        cursor: Option<&str>,
    ) -> Result<NextSnapshot, QueryFailure> {
        let now = Local::now().fixed_offset();
        let result = self
            .workspace
            .query_next(&NextQuery {
                root: self.root.clone(),
                limit: limit.unwrap_or(DEFAULT_NEXT_CANDIDATES),
                cursor: cursor.map(str::to_string),
                workspace_revision: self.revision,
                now,
            })
            .map_err(task_query_failure)?;
        let complete = result.completeness == QueryCompleteness::Complete;
        let provenance = match result.provenance {
            QueryProvenance::Memory => "memory",
            QueryProvenance::Persistent => "persistent",
            QueryProvenance::PersistentWithOverlay => "persistentWithOverlay",
        }
        .to_string();
        let NextResult {
            focused,
        ...
        Ok(NextSnapshot {
            ...
            skipped_invalid: skipped_invalid
                .into_iter()
                .map(|skipped| self.web_skipped_focus(skipped))
                .collect(),
            complete,
            provenance,
            documents: self.task_documents().map_err(|message| QueryFailure {
    ...
    fn web_skipped_focus(&self, skipped: NextSkippedFocus) -> WebSkippedFocus {
        WebSkippedFocus {
            path: display_path(&self.root, &skipped.path),
            id: skipped.id,
            title: skipped.title,
            codes: skipped
                .codes
                ...
                .collect(),
            location: SourceLocation::new(&self.root, &skipped.path, skipped.range),
        }
    }
