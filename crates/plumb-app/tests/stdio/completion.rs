...
fn completes_and_navigates_relative_verbatim_links_files_and_images() {
    ...
    assert!(diagnostics["params"]["diagnostics"]
        ...
        .unwrap()
        .iter()
        .any(|diagnostic| diagnostic["code"] == "embed.unresolved-file"));
    assert!(diagnostics["params"]["diagnostics"]
        .as_array()
        .unwrap()
        .iter()
...
fn completes_current_attributes_without_obsolete_resource_src() {
    ...
    let source =
        "`- Work\n\n `+ task\n\n `= created now\n `= pr\n\n`->{Alt path.png `+{embed} `={s}} `${\"x\" `={language t}}\n";
    let lines = source.lines().collect::<Vec<_>>();
    let priority_line = 5;
    let resource_line = 7;
    ...
    assert!(
        response(&output, 3)["result"].is_null(),
        "src is not a resource target property"
    );
