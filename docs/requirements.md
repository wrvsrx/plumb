...
严格性针对语法入口和结构良构，而不是要求每个标点在所有位置都必须转义。
...
属性和 modifier 必须贴在目标的结构边界上，让人和解析器都能清楚判断归属。

- block attribute 放在目标 block 前。
- span attribute 放在刚闭合的 span 后。
- list item attribute / task marker 放在 list marker 后的 item modifier 区。

这不追求表面位置完全一致，而追求结构归属一致。
...
- list item attribute 走 marker 后的 item modifier 区，避免 `- {.done}` 这种靠位置
  细节判断归属的写法。

## 7. Task model 需求

任务不是 core 语义。core 只解析 item modifier 或 item attribute；task extension 解释
状态、日期和查询字段。
...
- item-level 信息应统一进入 `@` modifier 通道。
...
再考虑增加 raw text span，用于一段里面有很多特殊拼写但不希望有 code 语义的文本：
