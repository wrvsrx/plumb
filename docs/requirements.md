...
- 普通段落不能凭空拥有缩进子块。
- heading 不是 indentation container；section tree 由 outline / section extension
  从 heading sequence 派生。
...
归属模型倾向改为 marker / form 内部 attr slot：
...
- @todo [due=2026-07-10 .urgent] 买牛奶
- @todo 买牛奶
```

- block attribute 在 block marker line 上。
- inline attribute 在 brace form 内。
- list item attribute 走 marker 后的 item modifier 区。
