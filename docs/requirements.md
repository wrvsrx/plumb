...
所有 marked block 统一为“marker line + 可选 subblocks”的形状：
...
block attr slot 必须紧贴 marker，中间不能有空格；marker 后的空格开始可选 head。这样
`#{#intro} 引言` 中的 `{#intro}` 无歧义地属于 heading，而 `# {em 引言}` 中的
`{em 引言}` 是标题文本里的 inline form。普通文本中的 `{` 仍遵循 inline brace form
规则。

head 是 marker line 上的 inline payload。对 list item、quote 和 generic container，head
作为首个 paragraph 的 surface sugar，归约后仍进入该节点的 `children`；缩进内容始终从
新的 child block 开始，不作为 head 的跨行 continuation。heading 的 head 直接成为标题
文本，并且 heading 不能借缩进获得 section children。内建 block 对 head 和 children 的
数量、类型限制属于 core structural validation，而不是 extension 语义。
...
属性值继续以 Pandoc / HTML 的形状为基线。block attr slot 使用紧贴 marker 的 braces：
