...
>           quote
:           generic container
Quotes      code block（one or more `"`, raw payload）
```
...
quote run 是保留的结构 marker。syntax parser 必须在 run 后根据 `{` / newline 与 `[` 分别
选择 block code 和 inline verbatim；这不要求 parser 解释 `language` 等 attributes。
...
这里第一个 item 的 `body_indent` 是 column 3，与 head `aaa` 的起始 column 对齐。grammar
...
- code quote marker 归约为 `CodeBlock { attr, text }`；quote count 只影响 source margin。
...
- 三个或更多连续反引号如何从左到右分组，以及剩余单个 introducer 的诊断。
- block start 的 introducer 在 marker、inline element 和 escape 之间的精确 dispatch 顺序。
